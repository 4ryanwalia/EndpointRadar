...
from report_module import build_threat_word_report, build_word_report
...
class EndpointDashboard(tk.Tk):
    def __init__(self) -> None:
        ...
        self.results: List[Dict[str, Any]] = scanner_module.load_scan_results()
        self.unauth_results: List[Dict[str, Any]] = scanner_module.load_unauthenticated_scan_results()
        ...
        self.subnet_var = tk.StringVar(value=scanner_module.guess_default_subnet())
    ...
    def _device_type_label(self, device: Dict[str, Any] | None) -> str:
        return str((device or {}).get("device_type_label") or scanner_module.describe_device_type(self._device_type(device)))
    ...
    def _device_from_scan(self, scan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "device_id": scanner_module.get_device_id(enriched_scan),
            "ip": ip_address,
            "hostname": hostname,
            ...
            "device_type_label": str(enriched_scan.get("device_type_label") or scanner_module.describe_device_type(enriched_scan.get("device_type"))),
    ...
    def _make_local_device_placeholder(self) -> Dict[str, Any]:
        try:
            system_info = scanner_module.os_check.get_system_info()
        except Exception:
            system_info = {}
        ...
        placeholder_scan = scanner_module.enrich_device_metadata(
            {
                "hostname": hostname,
        ...
        return {
            "device_id": scanner_module.get_device_id(placeholder_scan),
            "ip": ip_address,
            "hostname": hostname,
            "online": True,
            "status": "Local Device",
            ...
            "device_type_label": str(
                ...
                or scanner_module.describe_device_type(placeholder_scan.get("device_type"))
            ),
    ...
    def _make_placeholder_scan(self, device: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return scanner_module.enrich_device_metadata(placeholder)
    ...
    def _discover_devices_worker(self, subnet: str) -> None:
        try:
            devices = scanner_module.discover_devices(subnet)
            ...
            self._upsert_device(
                {
                    ...
                    "device_type_label": device.get("device_type_label") or scanner_module.describe_device_type(device.get("device_type")),
    ...
    def _unauthenticated_scan_worker(self, subnet: str) -> None:
        try:
            ...
            results = scanner_module.add_unauthenticated_scan_to_results(subnet, progress_callback=_progress)
    ...
    def _on_unauthenticated_scan_success(self, subnet: str, results: List[Dict[str, Any]]) -> None:
        self.unauth_scan_in_progress = False
        self.unauth_results = scanner_module.load_unauthenticated_scan_results()
        self._refresh_all()
        ...
        if results:
            self._select_device_by_key(scanner_module.get_device_id(results[0]))
        self._update_connection_status_ui()
    ...
    def _remote_scan_worker(
        ...
        try:
            ...
            if connection_entry:
                ...
                session = scanner_module.create_winrm_session(ip_address, str(username or ""), str(password or ""))
            ...
            scan = scanner_module.run_remote_scan_with_session(
                ip_address,
                session,
        ...
        except scanner_module.RemoteScanError as exc:
            self.after(
                0,
                lambda ip_address=ip_address, message=str(exc), code=exc.code, reuse_cached_session=reuse_cached_session: self._on_remote_scan_failure(
                    ip_address,
                    scanner_module.RemoteScanError(message, code=code),
            ...
            self.after(
                ...
                lambda ip_address=ip_address, wrapped_message=wrapped_message, reuse_cached_session=reuse_cached_session: self._on_remote_scan_failure(
                    ip_address,
                    scanner_module.RemoteScanError(wrapped_message, code="execution_error"),
                    reuse_cached_session,
                ),
            )
    ...
    def _on_remote_scan_success(self, scan: Dict[str, Any], ip_address: str, session: Any, username: str) -> None:
        ...
        scanner_module.upsert_scan_result(self.results, scan)
        scanner_module.save_scan_results(self.results)
        self._upsert_device(self._device_from_scan(scan))
        self._sync_devices_from_results()
        self._populate_system_list()
        self._update_remote_scan_progress_ui(
        ...
        self._select_device_by_key(scanner_module.get_device_id(scan))
        self._update_connection_status_ui()

    def _on_remote_scan_failure(
        self,
        ip_address: str,
        error: scanner_module.RemoteScanError,
        reuse_cached_session: bool = False,
    ) -> None:
        self.remote_scan_in_progress = False
        display_error = error
        if reuse_cached_session and error.code in {"invalid_credentials", "connection_failed", "timeout"}:
            ...
            display_error = scanner_module.RemoteScanError("Session expired, please reconnect.", code="session_expired")
    ...
    def _render_external_exposure_view(self, scan: Dict[str, Any] | None) -> None:
        ...
        empty_fields = {
            ...
            "open_ports": "-",
            "scan_type": "-",
            "visibility": scanner_module.LIMITED_VISIBILITY_NOTE,
        }
        ...
        self.external_exposure_title_label.config(
            text=f"Attacker View: {scanner_module.LIMITED_VISIBILITY_LABEL}",
            fg=TEXT,
        )
        self.external_exposure_note_label.config(
            text=str(exposure.get("limited_visibility_note") or scanner_module.LIMITED_VISIBILITY_NOTE),
            fg="#FFB74D",
        )
        ...
        field_values = {
            ...
            "scan_type": str(exposure.get("scan_type") or scanner_module.LIMITED_VISIBILITY_LABEL),
            "visibility": str(exposure.get("limited_visibility_note") or scanner_module.LIMITED_VISIBILITY_NOTE),
        }
    ...
    def _render_live_applications_view(self) -> None:
        ...
        if self.selected_scan and dashboard_data.is_limited_visibility_scan(self.selected_scan):
            self.live_apps_refresh_btn.config(state=tk.DISABLED)
            ...
            self._set_live_applications_status(
                scanner_module.LIMITED_VISIBILITY_NOTE,
                color="#FFB74D",
            )
            return
    ...
    def _request_live_application_refresh(self, auto_trigger: bool = False) -> None:
        ...
        if self.selected_scan and dashboard_data.is_limited_visibility_scan(self.selected_scan):
            message = scanner_module.LIMITED_VISIBILITY_NOTE
            self.live_apps_refresh_btn.config(state=tk.DISABLED)
            self._populate_live_processes_table([], "Live applications are not available in unauthenticated mode.")
    ...
    def _live_process_worker(self, ip_address: str, session: Any | None, is_local_scan: bool) -> None:
        try:
            if is_local_scan:
                processes = scanner_module.fetch_local_processes(limit=50)
            else:
                processes = scanner_module.fetch_live_processes_with_session(ip_address, session, limit=50)
            ...
            self.after(
                ...
                lambda ip_address=ip_address, message=str(exc): self._on_live_process_failure(
                    ip_address,
                    scanner_module.RemoteScanError(message, code="execution_error"),
                ),
            )
    ...
    def _on_live_process_failure(self, ip_address: str, error: scanner_module.RemoteScanError) -> None:
        self.live_process_fetch_in_progress = False
    ...
    def _clear_saved_devices(self) -> None:
        ...
        scanner_module.save_scan_results([])
        scanner_module.save_unauthenticated_scan_results([])
        self.results = []
        self.unauth_results = []
    ...
    def _update_component_charts(self, scan: Dict[str, Any]) -> None:
        ...
        contrib = dashboard_data.compute_contributions(scores)
        ...
        for comp in dashboard_data.COMPONENTS:
            ...
            if value > 0:
                labels.append(comp)
                sizes.append(value)
                colors.append(dashboard_data.COMPONENT_COLORS.get(comp, ACCENT))
        ...
        if sizes:
            ...
            self._pie_wedges = list(zip(wedges, labels, sizes))
        else:
        ...
        ordered = sorted([(k, float(contrib.get(k, 0.0))) for k in dashboard_data.COMPONENTS], key=lambda x: x[1], reverse=True)
        ...
        if bar_values:
            ...
            for name, val in ordered:
                base = dashboard_data.COMPONENT_COLORS.get(name, "#4FC3F7")
    ...
    def _update_summary_label(self) -> None:
        counts, _ = dashboard_data.risk_distribution(self.results)
    ...
    def _render_selected_scan(self) -> None:
        ...
        if exposure_scan is None and dashboard_data.is_limited_visibility_scan(scan):
            exposure_scan = scan
        scores = dashboard_data.compute_component_scores(scan)
        contrib = dashboard_data.compute_contributions(scores)
        ...
        self.why_label.config(text=dashboard_data.summarize_risk_reason(scan))
        drivers = dashboard_data.top_risk_drivers(scan, limit=3)
    ...
    def _build_antivirus_text(self, scan: Dict[str, Any]) -> str:
        if dashboard_data.is_limited_visibility_scan(scan):
            return (
                f"{scanner_module.LIMITED_VISIBILITY_LABEL}\n"
                f"{scanner_module.LIMITED_VISIBILITY_NOTE}\n\n"
                "Antivirus status is not visible in attacker-view mode."
            )
        av = scan.get("antivirus") or {}
        av_score = int(dashboard_data.compute_component_scores(scan).get("Antivirus", 0))
    ...
    def _build_ports_text(self, scan: Dict[str, Any]) -> str:
        ...
        if dashboard_data.is_limited_visibility_scan(scan):
            lines = [
                f"Scan Type: {scanner_module.LIMITED_VISIBILITY_LABEL}",
                scanner_module.LIMITED_VISIBILITY_NOTE,
                "",
                f"Monitored ports scanned: {', '.join(str(port) for port in sorted(scanner_module.UNAUTHENTICATED_PORT_MAP))}",
    ...
    def _render_installed_apps_table(self, scan: Dict[str, Any]) -> None:
        ...
        if dashboard_data.is_limited_visibility_scan(scan):
            if hasattr(self, "software_tree") and self.software_tree is not None:
                ...
                self.software_tree.insert(
                    "",
                    tk.END,
                    values=("Authentication Required", scanner_module.LIMITED_VISIBILITY_LABEL),
                    tags=("evenrow",),
                )
            ...
            if hasattr(self, "software_status_label"):
                self.software_status_label.config(
                    text=f"Third-party Software: {scanner_module.LIMITED_VISIBILITY_NOTE}",
                    fg="#FFB74D",
                )
            return
        ...
        if not apps:
            try:
                import installed_apps as installed_apps_module
        ...
        analysis = dashboard_data.analyze_software_risk(apps)
        risky_apps = dashboard_data.filter_risky_apps(analysis)
    ...
    def _build_software_text(self, scan: Dict[str, Any]) -> str:
        ...
        apps = scan.get("installed_apps") or []
        risky_apps = dashboard_data.filter_risky_apps(dashboard_data.analyze_software_risk(apps))
        if not apps:
    ...
    def _run_scan_clicked(self) -> None:
        try:
            new_scan = scanner_module.add_scan_to_results()
            self.results = scanner_module.load_scan_results()
            self._refresh_all()
            self._reset_remote_scan_progress_ui()
            self.discovery_status_label.config(text="Local scan completed.")
            self._select_device_by_key(scanner_module.get_device_id(new_scan))
    ...
    def _on_pie_click(self, event: Any) -> None:
        if not hasattr(self, "_pie_wedges") or event.inaxes != self.ax_pie:
            return
        for wedge, label, size in self._pie_wedges:
            ...
            if risky_ws is not None:
                ...
                for scan in self.results:
                    ...
                    if not isinstance(risky_apps, list):
                        risky_apps = dashboard_data.filter_risky_apps(
                            dashboard_data.analyze_software_risk(scan.get("installed_apps") or [])
                        )
