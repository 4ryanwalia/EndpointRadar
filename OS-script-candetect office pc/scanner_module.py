...
Lightweight wrapper around your existing endpoint scanner logic in os_check.py.
It exposes a clean function that:
- Runs a scan on the local system
- Normalizes the result into a standard structure
- Provides save/load helpers for multi-system results.
"""
...
import os_check  # reuses your existing scanning logic without changing it
import installed_apps


SCAN_RESULTS_FILE = Path("scan_results.json")
UNAUTH_SCAN_RESULTS_FILE = Path("unauth_scan_results.json")
REMOTE_SCAN_FILES = ["os_check.py", "installed_apps.py", "dashboard_data.py", "scanner_module.py"]
REMOTE_SCAN_DIR = r"C:\temp\endpoint_dashboard_remote_scan"
REMOTE_SCAN_RESULT_FILE = REMOTE_SCAN_DIR + r"\scan_result.json"
REMOTE_SCAN_RUNNER_FILE = REMOTE_SCAN_DIR + r"\scanner.py"
...
def _build_remote_runner_text() -> str:
    return (
        ...
        "result = scanner_module.run_single_scan()\n"
        ...
        payloads.append(
            {
                ...
                "remote_path": REMOTE_SCAN_DIR + "\\" + local_path.name,
...
def _verify_remote_file_exists(session: Any, remote_path: str, label: str) -> None:
    ...
    status_code, stdout, stderr = _run_remote_ps(session, script)
    if status_code != 0:
        message = stderr.strip() or stdout.strip() or f"Remote script transfer failed: {label}"
        ...
        for candidate_username in username_candidates:
            for transport in _build_winrm_transport_attempts():
                try:
                    ...
                    try:
                        setattr(session, "_endpoint_dashboard_username", candidate_username)
                    except Exception:
                        pass
...
def run_single_scan() -> Dict[str, Any]:
    """
    Runs a full scan using your existing functions from os_check.py and
    ...
    software_analysis = dashboard_data.analyze_software_risk(installed_inventory)
    risky_apps = dashboard_data.filter_risky_apps(software_analysis)
