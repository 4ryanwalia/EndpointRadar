...
def check_antivirus() -> dict:
    ...
    if code == 0 and out:
        ...
        cleaned = [ln for ln in lines if ln.lower() not in {"displayname productstate", "displayname productstate"}]
        products: list[str] = []
        states: list[int] = []
        for ln in cleaned:
...
def check_firewall_status() -> dict:
    ...
    any_off = False
    for prof, data in result["profiles"].items():
...
def main() -> None:
    ...
    if vulns:
        ...
        for v in vulns_sorted:
            ...
            label = "High Risk" if sev == "HIGH" else ("Medium Risk" if sev == "MEDIUM" else "Info")
