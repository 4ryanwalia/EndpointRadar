...
def _load_results() -> list[dict[str, Any]]:
    results = scanner.load_scan_results()
    risk_order = {"CRITICAL": 0, "HIGH": 1, "MEDIUM": 2, "LOW": 3, "UNKNOWN": 4}
    results.sort(
        key=lambda item: (
            risk_order.get(str(item.get("risk_level") or "UNKNOWN").upper(), 4),
            str(item.get("hostname") or scanner.get_device_id(item)).lower(),
        )
    )
    return results
...
def _device_identifier(scan: dict[str, Any]) -> str:
    return str(scanner.get_device_id(scan) or scan.get("target_ip") or scan.get("hostname") or "Unknown")
...
def _device_summary(scan: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "device_type_label": str(
            enriched_scan.get("device_type_label") or scanner.describe_device_type(enriched_scan.get("device_type"))
        ),
...
def _build_component_cards(scan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    contributions = risk.compute_contributions(scores)
    ...
    if not isinstance(risky_apps, list):
        risky_apps = risk.filter_risky_apps(risk.analyze_software_risk(scan.get("installed_apps") or []))
...
def _build_live_applications(scan: dict[str, Any]) -> tuple[list[dict[str, Any]], str]:
    if str(scan.get("connection_status") or "").strip().lower() == "local scan":
        try:
            return scanner.fetch_local_processes(limit=25), "Live snapshot collected from the local machine."
...
def device_details(device_id: str) -> Any:
    ...
    top_drivers = [
        ...
        for name, points, percent in risk.top_risk_drivers(scan, limit=3)
    ]

    payload = {
        "summary": _device_summary(scan),
        "risk_summary": risk.summarize_risk_reason(scan),
